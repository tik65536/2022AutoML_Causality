...
class SVM(SKLearnEstimator):
    def __init__(self,task='classification',**config):
        ...
        self.estimator_class=NuSVC

    @classmethod
    def search_space(cls,data_size,task):
        space={
            'nu':{'domain':tune.uniform(lower=0,upper=0.9),'init_value': 0.5 },
            'kernel':{'domain':tune.choice(['linear','poly','rbf']),'init_value':'rbf'},
            'degree':{'domain':tune.choice([1,2,3,4,5]),'init_value':3},
            'gamma':{'domain':tune.choice(['auto','scale']),'init_value':'scale'},
        }
        return space
